        ...
        if request.method == "POST":
            ...
            if 'unwatchlist' in request.POST:
                if item.watchlisted == True:
                    ...
                    item.save()
            if ('bid-submit' in request.POST) and (item.endBid == False):
                newBid = request.POST['newBid']
                item.itemBid = float(newBid)
                item.save()
            if 'bid-submit-end' in request.POST:
                item.endBid = True
                item.save()
            if 'comment-submit' in request.POST:
                comment = Comment.objects.create(item = item, user = request.user, text = request.POST['newComment'])
                comment.save()
    else:
...
def watchlist(request):
    items = Item.objects.filter(watchlisted = True)
...
def filtered(request, listingsCategory):
    items = Item.objects.filter(category = listingsCategory)
    return render(request, "auctions/customListings.html", {
        "items": items,
